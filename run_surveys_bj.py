...
for i in range(k_units.value.size):
    ...
    data_nl_bj, Beane_nl_bj, LSE_nl_bj, MCMC_nl_bj, N_nl_bj = analysis.keep_P_21(k_indices, spectra_sf_interp, params_sf,
                                            ...
                                            backend_filename=f'backends/survey_kmode_{k_HERA[i]:.2f}_sf_nl_bj_z{redshift:.3f}_int.h5',
    ...
    data_bj, Beane_bj, LSE_bj, MCMC_bj, N_bj = analysis.keep_P_21(k_indices, spectra_sf_interp, params_sf, n, model,
                                            ...
                                            backend_filename=f'backends/survey_kmode_{k_HERA[i]:.2f}_sf_bj_z{redshift:.3f}_int.h5',
                                            error_x=False, b_j_prior=True)

    np.savez(f'results_all_int/surveys/survey_kmode_{k_HERA[i]:.2f}_sf_nl_bj_z{redshift:.3f}_int', data=data_nl_bj, Beane=Beane_nl_bj, LSE=LSE_nl_bj,
                                        samples=MCMC_nl_bj[0], logp=MCMC_nl_bj[1], N=N_nl_bj, params=params_sf)
    np.savez(f'results_all_int/surveys/survey_kmode_{k_HERA[i]:.2f}_sf_bj_z{redshift:.3f}_int', data=data_bj, Beane=Beane_bj, LSE=LSE_bj,
