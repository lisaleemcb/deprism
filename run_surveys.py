...
for i in range(k_units.value.size):
    ...
    np.savez(f'results_all_int/surveys/survey_current_kmode_{k_HERA[i]:.2f}_sf_nl_z{redshift:.3f}_int', data=data_nl, Beane=Beane_nl, LSE=LSE_nl,
                                        samples=MCMC_nl[0], logp=MCMC_nl[1], N=N_nl, params=params_sf)
    np.savez(f'results_all_int/surveys/survey_current_kmode_{k_HERA[i]:.2f}_sf_z{redshift:.3f}_int', data=data, Beane=Beane, LSE=LSE,
